...
class GovernedPeopleIntelligenceAgent(PeopleIntelligenceAgent):
    ...
    def _render_derived_answer(spec: dict[str, Any], output: dict[str, Any]) -> str:
        ...
        def label(column: Optional[str]) -> str:
            return {'Dept': 'department', 'Location': 'location', 'Gender': 'gender', 'JobTitle': 'role',
                    'JobLevel': 'job level', 'Salary': 'salary', 'Tenure': 'tenure', 'Age': 'age',
                    'LastRating': 'performance rating', 'Attrition': 'recorded attrition'}.get(
                        column or '', (column or 'measure').replace('_', ' ').lower())
        ...
        filter_parts = []
        op_text = {'eq': '=', 'neq': '≠', 'lt': '<', 'lte': '≤', 'gt': '>', 'gte': '≥', 'in': 'in'}
        for item in spec.get('filters', []):
            value = item.get('value')
            shown = ', '.join(map(str, value)) if isinstance(value, list) else str(value)
            filter_parts.append(f"{label(item.get('column'))} {op_text.get(item.get('operator'), item.get('operator'))} {shown}")
        cohort = f"For {', '.join(filter_parts)}: " if filter_parts else ''
        ...
        if operation == 'compare_groups':
            ...
            if len(rows) >= 2:
                ...
                return cohort + f"{label(measure).capitalize()} comparison — {rows[0]['group']}: {value_text(float(rows[0]['value']))} (n={rows[0]['measured_count']}); {rows[1]['group']}: {value_text(float(rows[1]['value']))} (n={rows[1]['measured_count']}). Difference (second minus first): {value_text(difference)}."
        ...
        if operation == 'correlation':
            ...
            return cohort + f"The observed relationship between {label(spec.get('measure'))} and {label(spec.get('second_measure'))} is r={r:.2f} (n={n}, {p_text}). This is an association in the measured data, not evidence that one factor caused the other."
        ...
        if operation == 'crosstab':
            ...
            return cohort + f"PeopleOS calculated the {label(spec.get('group_by'))} by {label(spec.get('second_group_by'))} distribution. {shown} supported cells are available in the evidence details; {suppressed} small cells were hidden to protect against over-interpreting tiny groups."

        return cohort + 'PeopleOS completed a governed aggregate calculation. Open the evidence details to inspect the measured support.'

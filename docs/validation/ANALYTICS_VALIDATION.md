...
## All-engine and renderer review

The [engine and renderer audit](ENGINE_RENDERER_AUDIT.md) accounts for all **20 engine modules**, their public methods, API/schema interfaces, current and dormant Next consumers, Streamlit presentation, exports and shared agent evidence boundaries. The canonical [inventory](../../model/engine_audit.json) pins reviewed source hashes; CI checks roster completeness and rejects stale review evidence.

This pass fixes favorable-outcome fairness rates, missing-response and distribution denominators, optional-field merge reconciliation, current-active predictive coverage, positive-class probabilities, SHAP multioutput alignment, stale Cox/clustering state, topic schema validation, false prevalence/ROI/confidence claims and misleading renderer units. Causal and network engines now report unavailable directly, and the direct intervention fallback is retired. The legacy prediction tab and dormant individual-risk modal no longer imply validated future probabilities.

Verification: **255 selected Python regression checks**, **8 actual React server-render checks**, and **9 pinned public benchmark assertions** pass locally. The production frontend build passes for 16 static pages; lint reports **0 errors and 22 warnings**. Server-render checks validate numbers, units and evidence states, not browser interactions or pixel-level layout. The existing Windows packaging/launcher path is unchanged; release verification still requires the three packaged smoke jobs on the published PR head.

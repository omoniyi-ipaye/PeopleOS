...
| Agent behavior | Missing tool evidence, unsupported scopes, omitted requested metrics and sparse measurements could appear complete | Required-metric and scope checks; warnings and denominators retained; completed analytical tool results are handed to the local model for citation-grounded People-language synthesis, with server validation and deterministic factual fallback |
...
The application agent remains a governed aggregate, read-only investigator with deterministic planning and an optional local narrative agent. The analytical tools complete before the model receives the full computed evidence bundle; the model may select and combine supplied results, but cannot calculate, mutate data, call arbitrary tools or make employment decisions. Server-side citation, scope and policy checks preserve a deterministic factual fallback. Representative future-outcome ML validation, actual LLM grounding/injection/latency evaluation, workload/recovery objectives, individual enterprise identity and retention requirements, broader accessibility testing, and a real People-team pilot remain required before stronger readiness claims.

...
def test_evidence_bundle_flattens_tool_evidence():
    item = EvidenceItem(
        ...
        claim="Engineering turnover is above baseline",
        source_tool="workforce.turnover",
        value=0.187,
        metric="turnover_rate",
        confidence=0.91,
    )
    result = ToolResult(
        tool_id="workforce.turnover",
        status=ToolResultStatus.SUCCESS,
        summary="Turnover analysis completed",
        evidence=[item],
    )
    bundle = EvidenceBundle(
        question="Why is Engineering turnover increasing?",
        tool_results=[result],

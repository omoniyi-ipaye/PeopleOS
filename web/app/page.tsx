...
export default function DecisionCockpitPage() {
  ...
  return <div className="mx-auto max-w-[1440px] space-y-6 pb-10">
    <section className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><div className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-violet-600 dark:text-violet-300">Decision cockpit</div><h1 className="text-3xl font-semibold tracking-tight text-slate-950 dark:text-white md:text-4xl">What deserves your attention?</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-400">Start with what the data actually supports, then move into evidence, investigation or exploratory planning. Metric and model boundaries stay visible throughout.</p></div><Link href="/advisor" className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-100"><Brain className="h-4 w-4" /> Ask PeopleOS</Link></section>

    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Active workforce" value={activeCount.toLocaleString()} note="current employees in the resolved population" icon={Users} /><Metric label="Observed attrition share" value={percentage(observedAttritionShare)} note="recorded outcome share; not period turnover" icon={TrendingDown} /><Metric label="Average active tenure" value={tenure ? `${tenure.toFixed(1)}y` : '—'} note="current active workforce" icon={Activity} /><Metric label="Trust state" value={status?.workspace?.active_model ? 'Model active' : 'Evidence-first'} note={status?.capabilities?.llm ? 'local synthesis available' : 'deterministic synthesis'} icon={ShieldCheck} /></section>

    <section className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]"><div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900 md:p-7"><div className="mb-5 flex items-start justify-between gap-4"><div><div className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Priority briefing</div><h2 className="mt-1 text-xl font-semibold text-slate-950 dark:text-white">Signals worth reviewing now</h2></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">Current dataset active</span></div><div className="space-y-3">{signals.map((signal) => <Link key={signal.title} href={signal.href} className="group flex items-start gap-4 rounded-2xl border border-slate-200/80 p-4 transition hover:border-violet-200 hover:bg-violet-50/40 dark:border-white/10 dark:hover:border-violet-500/20 dark:hover:bg-violet-500/[0.04]"><div className={`mt-0.5 grid h-10 w-10 shrink-0 place-items-center rounded-xl ${signal.tone === 'attention' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300' : 'bg-slate-100 text-slate-600 dark:bg-white/5 dark:text-slate-300'}`}><signal.icon className="h-5 w-5" /></div><div className="min-w-0 flex-1"><div className="font-semibold text-slate-900 dark:text-white">{signal.title}</div><div className="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-400">{signal.detail}</div><div className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-violet-600 group-hover:gap-2 dark:text-violet-300">{signal.action}<ArrowRight className="h-3.5 w-3.5" /></div></div></Link>)}</div></div>

      <div className="space-y-6"><div className="rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-950 to-slate-900 p-6 text-white shadow-xl dark:border-white/10"><div className="mb-5 grid h-11 w-11 place-items-center rounded-2xl bg-violet-500/15 text-violet-300 ring-1 ring-violet-400/20"><Brain className="h-5 w-5" /></div><div className="text-xs font-bold uppercase tracking-[0.16em] text-violet-300">Investigate</div><h2 className="mt-2 text-xl font-semibold">Ask what the evidence supports.</h2><p className="mt-2 text-sm leading-6 text-slate-300">People Intelligence selects governed tools, exposes its evidence ledger, separates coverage from heuristic evidence quality, and refuses unsupported inference.</p><Link href="/advisor" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-slate-100">Start an investigation <ArrowRight className="h-4 w-4" /></Link></div><div className="rounded-3xl border border-slate-200 bg-white p-6 dark:border-white/10 dark:bg-slate-900"><div className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Capability state</div><div className="mt-4 space-y-3 text-sm"><StateRow label="Dataset" value="Active" good /><StateRow label="Predictive model" value={status?.workspace?.active_model ? 'Active' : 'Not active'} good={Boolean(status?.workspace?.active_model)} /><StateRow label="Synthesis" value={status?.capabilities?.llm ? 'Local model' : 'Deterministic fallback'} good /></div><Link href="/platform" className="mt-5 inline-flex items-center gap-1 text-xs font-semibold text-violet-600 dark:text-violet-300">Open Trust Center <ArrowRight className="h-3.5 w-3.5" /></Link></div></div></section>
...
function StateRow({ label, value, good }: { label: string; value: string; good: boolean }) { return <div className="flex items-center justify-between gap-4"><span className="text-slate-500 dark:text-slate-400">{label}</span><span className="inline-flex items-center gap-2 font-medium text-slate-800 dark:text-slate-200"><span className={`h-2 w-2 rounded-full ${good ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`} />{value}</span></div> }

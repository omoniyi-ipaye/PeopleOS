...
export default function DecisionCockpitPage() {
  ...
  const { data: summary, isLoading: summaryLoading } = useQuery<AnalyticsSummary>({
    ...
    queryFn: () => api.analytics.getSummary() as Promise<AnalyticsSummary>,
    enabled: hasData,
  })
  ...
  if (!hasData) {
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-4xl items-center justify-center p-4">
        <div className="w-full rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/40 dark:border-white/10 dark:bg-slate-900 dark:shadow-none md:p-12">
          ...
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-base leading-7 text-slate-600 dark:text-slate-300">PeopleOS will validate the data, tell you what can be analysed immediately, explain which capabilities need a trained model, and then build your first decision briefing.</p>
          </div>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/upload" className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-violet-600/20 transition hover:bg-violet-500">Add workforce data <ArrowRight className="h-4 w-4" /></Link>
            <Link href="/advisor" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-white/10 dark:text-slate-200 dark:hover:bg-white/5">See how investigations work</Link>
          </div>
        </div>
      </div>
    )
    ...
    <div className="mx-auto max-w-[1440px] space-y-6 pb-10">
      <section className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-400">Start with the signal, then move into evidence, investigation, or planning. PeopleOS keeps model and data state visible throughout.</p>
        </div>
        <Link href="/advisor" className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-100"><Brain className="h-4 w-4" /> Ask PeopleOS</Link>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900 md:p-7">
          <div className="mb-5 flex items-start justify-between gap-4"><div><div className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Priority briefing</div><h2 className="mt-1 text-xl font-semibold text-slate-950 dark:text-white">Signals worth reviewing now</h2></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">Dataset active</span></div>
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-950 to-slate-900 p-6 text-white shadow-xl dark:border-white/10"><div className="mb-5 grid h-11 w-11 place-items-center rounded-2xl bg-violet-500/15 text-violet-300 ring-1 ring-violet-400/20"><Brain className="h-5 w-5" /></div><div className="text-xs font-bold uppercase tracking-[0.16em] text-violet-300">Investigate</div><h2 className="mt-2 text-xl font-semibold">Ask why, not just what.</h2><p className="mt-2 text-sm leading-6 text-slate-300">People Intelligence selects governed tools, shows the evidence it used, and tells you when the evidence is incomplete.</p><Link href="/advisor" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-slate-100">Start an investigation <ArrowRight className="h-4 w-4" /></Link></div>
          <div className="rounded-3xl border border-slate-200 bg-white p-6 dark:border-white/10 dark:bg-slate-900"><div className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Capability state</div><div className="mt-4 space-y-3 text-sm"><StateRow label="Dataset" value="Active" good /><StateRow label="Predictive model" value={status?.workspace?.active_model ? 'Active' : 'Not active'} good={Boolean(status?.workspace?.active_model)} /><StateRow label="AI synthesis" value={status?.capabilities?.llm ? 'Local model' : 'Deterministic fallback'} good /></div><Link href="/platform" className="mt-5 inline-flex items-center gap-1 text-xs font-semibold text-violet-600 dark:text-violet-300">Open Trust Center <ArrowRight className="h-3.5 w-3.5" /></Link></div>
        </div>
      </section>
...
function StateRow({ label, value, good }: { label: string; value: string; good: boolean }) {
  return <div className="flex items-center justify-between gap-4"><span className="text-slate-500 dark:text-slate-400">{label}</span><span className="inline-flex items-center gap-2 font-medium text-slate-800 dark:text-slate-200"><span className={`h-2 w-2 rounded-full ${good ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`} />{value}</span></div>
}

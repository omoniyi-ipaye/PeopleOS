...
export default function DecisionCockpitPage() {
  ...
  return <div className="mx-auto max-w-[1440px] space-y-6 pb-10">
    <section className="flex flex-wrap items-start gap-x-6 gap-y-4"><div className="min-w-0 max-w-3xl flex-1 basis-[22rem]"><div className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-violet-600 dark:text-violet-300">Decision cockpit</div><h1 className="text-3xl font-semibold tracking-tight text-slate-950 dark:text-white md:text-4xl">What deserves your attention?</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-400">Start with the workforce signals that matter. Evidence boundaries stay available without competing with the decision itself.</p>{status?.integrity?.snapshot && <div className="mt-3 flex w-fit max-w-full flex-wrap items-center gap-x-3 gap-y-2 text-xs text-slate-500 dark:text-slate-400"><span className="inline-flex min-w-0 max-w-full items-center gap-1.5"><Database className="h-3.5 w-3.5 shrink-0 text-slate-400" /><span className="truncate">Source: <strong className="text-slate-700 dark:text-slate-200">{status.integrity.snapshot.source_name ?? 'Current workforce dataset'}</strong>{status.integrity.snapshot.dataset_version != null ? ` · v${status.integrity.snapshot.dataset_version}` : ''}</span></span><span className={`inline-flex items-center gap-1.5 font-semibold ${status.integrity.status === 'verified' ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300'}`} role="status"><ShieldCheck className="h-3.5 w-3.5" />{status.integrity.status === 'verified' ? 'Dataset snapshot verified' : 'Dataset snapshot needs attention'}</span><Link href="/upload" className="inline-flex items-center gap-1 font-semibold text-violet-600 dark:text-violet-300">Manage data source <ArrowRight className="h-3.5 w-3.5" /></Link></div>}</div><Link href="/advisor" aria-label="Start an investigation" className="inline-flex shrink-0 self-start items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-100"><Brain className="h-4 w-4" /> Ask PeopleOS</Link></section>

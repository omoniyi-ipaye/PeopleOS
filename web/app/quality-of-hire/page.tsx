  ...
  return (
    <Page>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button size="sm" variant={tab === 'sources' ? 'primary' : 'secondary'} onClick={() => setTab('sources')}><Target className="h-4 w-4" />Source cohorts</Button>
      ...
      {tab === 'overview' && <>
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          <MetricCard label="People represented" value={(summary?.total_employees ?? 0).toLocaleString()} detail="Current historical outcome sample" icon={UserPlus} />
          <MetricCard label="Source cohorts" value={(summary?.sources_analyzed ?? 0).toLocaleString()} detail="Recruiting channels meeting analysis thresholds" icon={Target} />
          <MetricCard label="Pre-hire measures" value={(summary?.prehire_signals_count ?? 0).toLocaleString()} detail="Candidate-stage variables available for association screening" icon={BarChart3} />
        </section>
      ...
      {tab === 'sources' && <Surface padding="lg">
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b border-border text-[11px] uppercase tracking-wider text-text-muted"><th className="py-3 pr-4 text-left">Source</th><th className="px-4 py-3 text-right">Hires</th><th className="px-4 py-3 text-right">Avg rating</th><th className="px-4 py-3 text-right">Observed retained share</th><th className="py-3 pl-4 text-right">Heuristic composite</th></tr></thead><tbody>{sources.map((source) => <tr key={source.HireSource} className="border-b border-border last:border-0"><td className="py-3 pr-4"><div className="font-medium">{source.HireSource}</div><div className="text-xs text-text-muted">{source.recommendation}</div></td><td className="px-4 py-3 text-right">{source.hire_count}</td><td className="px-4 py-3 text-right">{source.avg_performance?.toFixed(2) ?? '—'}</td><td className="px-4 py-3 text-right">{source.retention_rate_pct == null ? '—' : `${source.retention_rate_pct.toFixed(0)}%`}</td><td className="py-3 pl-4 text-right"><StatusBadge tone="neutral">{source.quality_score?.toFixed(1) ?? '—'}</StatusBadge></td></tr>)}</tbody></table>
      ...
      {tab === 'associations' && <Surface padding="lg">

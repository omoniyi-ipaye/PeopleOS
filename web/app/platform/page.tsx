...
interface LanguageModelStatus { enabled: boolean; ready: boolean; selected_model: string; setup_state?: string; reason?: string | null }
...
export default function TrustCenterPage() {
  ...
  const [languageModel, setLanguageModel] = useState<LanguageModelStatus | null>(null)
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const responses = await Promise.all([fetch('/api/platform/workspaces/local'), fetch('/api/platform/health'), fetch('/api/platform/workspaces/local/fitness'), fetch('/api/platform/me'), fetch('/api/llm/status')])
      if (!responses.slice(0, 4).every(response => response.ok)) throw new Error('PeopleOS could not verify its trust state.')
      setWorkspace(await responses[0].json()); setHealth(await responses[1].json()); setFitness(await responses[2].json()); setActor(await responses[3].json()); setLanguageModel(responses[4].ok ? await responses[4].json() : null); setError(null)
  ...
  return <Page>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <MetricCard label="Predictive model" value={activeModel ? 'Experimental model active' : 'Not part of this release'} detail={activeModel ? activeModel.model_family : 'Aggregate evidence analysis remains available'} icon={Brain} tone={activeModel ? 'info' : 'neutral'} />
    ...
    <Surface padding="lg">
      ...
      {advanced && <div id="advanced-trust-details" className="mt-6 border-t border-border pt-6"><div className="grid gap-6 lg:grid-cols-2"><div><div className="text-xs font-semibold uppercase tracking-wider text-success">Can recover automatically</div><ul className="mt-3 space-y-2 text-sm leading-6 text-text-secondary">{health?.autonomous_recovery_envelope?.map(item => <li key={item}>• {item}</li>)}</ul></div><div><div className="text-xs font-semibold uppercase tracking-wider text-warning">Needs an explicit action</div><ul className="mt-3 space-y-2 text-sm leading-6 text-text-secondary">{health?.governed_only?.map(item => <li key={item}>• {item}</li>)}</ul></div></div><div className="mt-6 rounded-2xl bg-background-secondary p-4 font-mono text-xs leading-6 text-text-muted">Dataset: {activeDataset?.dataset_id ?? 'none'}<br />Predictive model: {activeModel?.model_id ?? 'none'}<br />Local language model: {languageModel?.ready && languageModel.enabled ? languageModel.selected_model : 'optional / not active'}<br />Workspace: {workspace?.workspace_id ?? 'local'}</div></div>}
    </Surface>

...
export interface SurvivalAnalysisResult {
    ...
    hazard_over_time?: {
        available: boolean
        reason?: string
        hazard_over_time?: Array<{ time_years: number; baseline_hazard: number; cumulative_hazard: number; survival: number }>
        risk_periods?: Array<{ time_years: number; relative_risk: number; interpretation: string }>
    }
...
export interface QualityOfHireAnalysisResult {
    ...
    retention_correlations?: QoHCorrelationsResult
    insights: {
        available?: boolean
        best_source?: string
...
export interface SpanAnalysis {
    available: boolean
    overall_stats: { total_managers: number; avg_span: number; median_span: number; under_span_pct: number; optimal_pct: number; over_span_pct: number }
...
export interface PromotionEquityResult {
    ...
    results?: Array<{ group: string; count: number; avg_promotion_velocity: number; gap_vs_reference: number; pct_promoted: number }>
    statistical_test?: { test_type: string; statistic: number; p_value: number; is_significant: boolean }
    summary?: string
    recommendations?: string[]
}

export interface StructuralAnalysisResult {
    stagnation: { hotspots: StagnationHotspot[]; at_risk_employees: StagnantEmployee[]; summary: { total_analyzed: number; stagnant_count: number; stagnation_pct: number; avg_stagnation_index: number } }
    span_of_control: SpanAnalysis
    promotion_equity: PromotionEquityResult
    promotion_bottlenecks: Array<{ job_level: string; avg_wait_years: number; bottleneck_severity: string }>
    recommendations: string[]
    warnings: string[]
}
...
export interface ENPSResult {
    available: boolean
    reason?: string
    overall?: { enps_score: number; promoters_pct: number; passives_pct: number; detractors_pct: number; total_responses: number; interpretation: string }
    by_group?: Record<string, { enps_score: number; response_count: number; promoters_pct: number; detractors_pct: number }>
    trends?: Array<{ period: string; enps_score: number; response_count: number; change_from_prior?: number }>
    drivers?: Array<{ dimension: string; correlation: number; impact: 'Strong' | 'Moderate' | 'Weak'; recommendation: string }>
    improvement_areas?: Array<{ dimension: string; avg_score: number; benchmark: number; gap: number; priority: 'High' | 'Medium' | 'Low' }>
}

export interface OnboardingTrajectory {
    EmployeeID: string
    surveys: Array<{ survey_type: string; score: number; date: string }>
...
export interface OnboardingHealth {
    ...
    trends?: Array<{ survey_type: string; avg_score: number; response_count: number }>
}
...
export interface SentimentAnalysisResult {
    enps: ENPSResult
    onboarding: { available: boolean; health: OnboardingHealth; trajectories: OnboardingTrajectory[] }
    early_warnings: EarlyWarning[]
    summary: { enps_available: boolean; onboarding_available: boolean; overall_sentiment: 'Positive' | 'Neutral' | 'Negative'; critical_warnings_count: number }
    recommendations: string[]
    warnings: string[]
}
